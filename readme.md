...
1. **Clone the repository:**
   ```bash
   git clone https://github.com/0xBoji/go-quant-backtestdata.git
   cd GO-Microstructures
   ```

2. **Configure Environment:**
   Ensure `GOAMD64=v4` is set to leverage AVX-512 on Zen 4 CPUs.

3. **Data Management:**
   Historical data is stored in the `data/` directory (ignored by git). Use the `data` command to populate it.