...
// runQuantiles computes distribution quantiles for Kalman_Adaptive and Pressure_TCI
// for a given symbol using the same GNC + MathContext stack.
func runQuantiles(sym string) {
	fmt.Printf("\n>>> QUANTILES STUDY: %s (Kalman_Adaptive, Pressure_TCI) <<<\n", sym)

	// Collect tasks (days) for this symbol.
	var tasks []ofiTask
	for t := range discoverTasks(sym) {
		tasks = append(tasks, t)
	}
	if len(tasks) == 0 {
		fmt.Printf("[warn] No tasks found for symbol %s in %s\n", sym, BaseDir)
		return
	}
	...
	var kalVals []float64
	var presVals []float64

	for _, task := range tasks {
		gncBlob, ok := loadRawGNC(sym, task, &gncBuf)
		if !ok || len(gncBlob) == 0 {
			continue
		}
		...
		kalBuf := make([]float64, n)
		presBuf := make([]float64, n)

		var kal GenAdaptiveKalman
		kal.Reset()
		kal.Update(mathCtx, kalBuf)

		var pres GenLeakyIntegrator
		pres.Reset()
		pres.Update(mathCtx, presBuf)

		kalVals = append(kalVals, kalBuf...)
		presVals = append(presVals, presBuf...)
	}

	if len(kalVals) == 0 || len(presVals) == 0 {
		fmt.Println("[warn] No signal values collected; cannot compute quantiles")
		return
	}

	probs := []float64{0.90, 0.95, 0.99}
	qKal := computeQuantiles(kalVals, probs)
	qPres := computeQuantiles(presVals, probs)

	fmt.Printf("Kalman_Adaptive quantiles (Q90,Q95,Q99): %.4f %.4f %.4f\n", qKal[0], qKal[1], qKal[2])
	fmt.Printf("Pressure_TCI  quantiles (Q90,Q95,Q99): %.4f %.4f %.4f\n", qPres[0], qPres[1], qPres[2])
}

// computeQuantiles returns empirical quantiles for given probabilities in [0,1].
func computeQuantiles(values []float64, probs []float64) []float64 {
	res := make([]float64, len(probs))
	if len(values) == 0 {
		return res
	}
